...
Usd = 8;
Hole = Ly*Lx*2/8;
D_values = [7000,10000,12000,15000,18000,20000];
trunc_errs = [  1.74e-07,1.05e-07, 7.99e-08,5.61e-08 ,  4.14e-08, 3.46e-08]'*1e8;
...
nns_extraplt = nns_extraplt * 0.1;
positiveIndices = nns_extraplt >= 0;
negativeIndices = nns_extraplt < 0;
h3 = loglog(x_values(positiveIndices), nns_extraplt(positiveIndices), 's','MarkerSize', 6); hold on;
h3_negative = loglog(x_values(negativeIndices), -nns_extraplt(negativeIndices), 's','MarkerSize', 6); hold on;
...
for band = 0
    ...
    if(band == 0)
        h_fit3 = loglog(x_guide, y_guide, '--', 'LineWidth', 1.5);
    end
end
