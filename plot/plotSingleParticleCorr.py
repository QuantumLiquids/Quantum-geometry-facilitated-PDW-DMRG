...
for D in D_values:
    ...
    file_path = f"../data/bupcbupa{Ly}x{Lx}ts{ts}td{td}tsd_xy{tsd_xy}tsd_nn{tsd_nn}Uss{Uss}Udd{Udd}Usd{Usd}Hole{Hole}D{D}.json"
...
plt.xlabel(r'$x$')
plt.ylabel(r'-$\langle c^\dagger(0) c(x) \rangle$')
#plt.ylim(1e-5, 1e-1)  
plt.show()
